  ...
  const [isLoading, setIsLoading] = useState<boolean>(true);
    ...
    useEffect(() => {
        const fetchCartItems = async () => {
            try {
                const items = await CartService.getCart("eb4bef7e-e85a-4395-8302-ee2e6ad894f6");
                setCartItems(items);
                setIsLoading(false);
            } catch (error) {
    ...
    <div className="cart-container">
        ...
        cartItems.items.map((item) => (
            ...
            product={{
                ...
                category_id: "cat1", // tu peux adapter selon tes données
        ...
        {cartItems.items.length > 0 && (
        <Button label="Passer à la caisse" onClick={() => console.log("Caisse")} />
        )}
    </div>
    );
}
